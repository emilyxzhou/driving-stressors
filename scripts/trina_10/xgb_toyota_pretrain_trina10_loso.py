...
# LOSO, training on trina_33 + all but one trina_10 subject
for i, (train_index, test_index) in enumerate(loso.split(unique_subjects)):
    ...
    X_train_t10 = X_test[train_indices, :][0]
    y_train_t10 = y_test[train_indices]
    X_test_t10 = X_test[test_indices, :][0]
    y_test_t10 = y_test[test_indices]

    X_train_all = np.concatenate((X_train, X_train_t10), axis=0)
    y_train_all = np.concatenate((y_train, y_train_t10), axis=0)
    ...
    test_zeros = np.count_nonzero(y_test_t10 == 0)
    test_ones = np.count_nonzero(y_test_t10 == 1)

    y_proba = model.predict_proba(X_test_t10)
    y_pred = np.argmax(y_proba, axis=1)
    y_proba = y_proba[:, 1]

    auroc = roc_auc_score(y_test_t10, y_proba)
    ...
    y_pred[y_pred < 0.5] = 0
    y_pred[y_pred >= 0.5] = 1
    acc = accuracy_score(y_test_t10, y_pred)
    print(f"Acc: {acc:.4f}")
    scores.append(acc)

    # Save the predictions
    probas_0 = y_proba[np.where(y_test_t10 == 0)[0]]
    probas_1 = y_proba[np.where(y_test_t10 == 1)[0]]
